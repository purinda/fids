...
begin
    ...
    begin
        if not (Key in [#8,#32, 'a'..'z', 'A'..'Z']) then begin
            // Discard the key
            Key := #0;
        end;
    end;
...
begin
    ...
    begin
        ...
        begin
            ...
            if (lowercase(FieldName) = 'off blocks') OR (lowercase(FieldName) = 'on blocks') then   // 4 row 1 column
            begin
                lblField[I].Top := Row4;
                lblField[I].Left := Col1;
            end;
            if (lowercase(FieldName) = 'atd') OR (lowercase(FieldName) = 'ata') then   // 4 row 2 column
            begin
                lblField[I].Top := Row4;
                lblField[I].Left := Col2;
            end;
            ...
            if (lowercase(FieldName) = 'scheduled checkin counters') then  // 8 row 1 column
            begin
                lblField[I].Top := Row8;
                lblField[I].Left := Col1;
            ...
            if (lowercase(FieldName) = 'checkin opening time') then   // 8 row 2 column
            begin
                lblField[I].Top := Row8;
                lblField[I].Left := Col2;
                lblField[I].AutoSize := false;
            ...
            if (lowercase(FieldName) = 'checkin closing time') then  // 8 row 3 column
            begin
                lblField[I].Top := Row8;
                lblField[I].Left := Col3;
                lblField[I].AutoSize := false;
        ...
        case ffField of
            ...
            ffScheduledCheckinCounters:
                begin
                    ...
                    inpField[I].SetParentComponent(self);
                    TEdit(inpField[I]).Text := FlightData.ScheduledCheckinCounters;
                    TEdit(inpField[I]).OnKeyPress := EditKeyPress;
