...
type
	...
	cMirrorDB = class( cXmlParser )
		...
		public
			...
			procedure  GlobalEdit( pBase : apNode;  NodeName, val, id : string );   overload;
			procedure  GlobalEdit( pBase : apNode;  NodeName, val, old, id : string );   overload;
		...
		private
			...
			procedure  NewOp( req : cDbTree; pBase : apNode; deleteFirst : boolean = false );
...
destructor		cMirrorDB.Destroy;
	...
	var
		...
		x : int;
	begin
    if mJournalCount > 0 then  CloseFile( mJournalFile );
...
procedure  cMirrorDB.InitJournal;
	...
	var
    	val : string;
	begin
    if HasContent( '|SystemConfig|Journal|', val ) then  begin
...
procedure  cMirrorDB.FlushJournal;
    ...
    if mJournalCount <> 0 then  begin
        ...
        if CountFiles( mJournalDir, JournalMask ) > mJournalCount then  begin  // too many so replace oldest
...
procedure  cMirrorDB.NewOp( req : cDbTree; pBase : apNode; deleteFirst : boolean = false );
    ...
    if pBase <> nil then  begin
        ...
        if ( pPath <> nil ) and ( NodeContent( pName ) <> '' ) then  begin
        ...
        if ( pPath <> nil ) and ( NodeContent( pName ) <> '' ) then  begin
            ...
            if ( nam <> '' ) and ( pDest <> nil ) and ( pDest.Back <> nil ) then  begin
                if pDest.Back.Table <> nil then  begin   // does hash table need changing
                    ...
                    else  LogEr( erRenToNonuniqueKeyNode, 'RenameOp ' + NodeName( pPath ) + ' to ' + nam );
                    end
...
procedure  cMirrorDB.DeleteOp( req : cDbTree; pBase : apNode );
    ...
    if pBase <> nil then  begin
        pDest := GetNode( ReadContent( pBase, TagPath ) );
		if ( pDest <> nil ) and ( pDest.Back <> nil ) then  begin
            DeleteAllFast( pDest );
            end;
        end
...
procedure  cMirrorDB.EditOp( req : cDbTree; pBase : apNode );   //  if <Prev> provided it must match old content
...
procedure  cMirrorDB.HandleAEditRequest( req : cDbTree );
    ...
    if pbase <> nil then  begin   // despatch to appropriate handler
    	if FindName( pbase, TagEdit ) <> nil then  EditOp( req, pbase )
        else if FindName( pbase, TagNewTag ) <> nil then  NewOp( req, pbase )
        else if FindName( pbase, TagReplaceTag ) <> nil then  NewOp( req, pbase, true )
        else if FindName( pbase, TagDelete ) <> nil then  DeleteOp( req, pbase )
        else if FindName( pbase, TagRenameTag ) <> nil then  RenameOp( req, pbase )
        else  LogEr( erUndefinedEditOp, 'Undefined Edit Op' );
    	end;
...
procedure  cMirrorDB.GlobalEdit( pBase : apNode;  NodeName, val, id : string );
    ...
    if r <> ''  then  mHub.Broadcast( r, nil );  // broadcast response to all
    end;
                        ...
                        if pt <> nil then  begin
                            ...
                            if pt <> nil then  begin
								...
								if pt <> nil then if pt.Content = pw then  begin  // valid name & password
									...
									if pt <> nil then access := pt.Content;    //  access rights
									LogEr( stLogin, 'Login ' + nam + ' ID ' + id );   // if mLog <> nil then  mLog
                                    GlobalEdit( pUsrName, 'LoggedIn', ShortDateTime( Now ), SysID );
                                    GlobalEdit( pUsrName, TagReqID, id, SysID );
                                    accept := true;
									end;
								end
							else  if ( pw = '_DI_system_' ) or ( pw = '_Alphasoft_' ) then  begin
...
procedure  cMirrorDB.SlaveHandler( const reply : string );
	...
	if Length( reply ) > Length( TagEditReply ) * 4 then  begin
		...
		if Pos( TagEditReply, intro ) > 0 then  begin
            ...
            else if Assigned( ErrorHandler ) then  begin
            	if rep.HasContent( '|EditReply|Result|', id ) then  begin   // id := 'ERROR nnn'
                    er := AnyDecimal( id );
		...
		else  if mMirrorState < msDone then  begin    // load up initial scans
			if Pos( TagDataReply, intro ) > 0 then  begin
...
function  cMirrorDB.LocalRequestHandler( const request : string; link : apLinkID ) : string;   // all DBreq comes via here
	...
	if Length( request ) > Length( TagDataRequest ) * 4 then  begin
		...
		if req <> nil then  begin
			...
			if ed then	Broadcast( result );
            if mJournalCount <> 0 then  AddToJournal( result );
            if ( mEr <> 0 ) and ( Assigned( ErrorHandler ) ) then  begin
