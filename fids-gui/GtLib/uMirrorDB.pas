...
type
	...
	cMirrorDB = class( cXmlParser )
		...
		private
			...
			procedure  NewOp( req : cDbTree; pBase : apNode; lPath : TStringList; deleteFirst : boolean = false );
...
destructor		cMirrorDB.Destroy;
	...
	var
		...
		x : int;
	begin
    if mJournalCount > 0 then  try CloseFile( mJournalFile );   except  end;
    FreeAndNil( oLoggedIn );
    FreeAndNil( oMirrorList );
    for x := 0 to oDeltaNotify.Count - 1 do  begin
		rp := oDeltaNotify[ x ];
		Dispose( rp );
        end;
...
procedure	cMirrorDB.ClearAllReaders;
	...
	var
		...
		x : int;
	begin
	if oDeltaNotify <> nil then  begin
		for x := 0 to oDeltaNotify.Count - 1 do  begin
			rp := oDeltaNotify[ x ];
            Dispose( rp );
            end;
    ...
    if not mEntered then  begin
        try  begin
            ...
            if oDeltaNotify <> nil then  begin
                ...
                while ( i >= 0 ) and ( i < oDeltaNotify.Count ) do  begin  // tolerant to list changing
                    p := oDeltaNotify[ i ];
                    try      p^( xml );
                    finally  Dec( i );   end;
                    end;
...
procedure  cMirrorDB.InitMirror( paths : TStringList );
	...
	else  oMirrorList.Add( LSep );    // default is root. ie all
...
procedure  cMirrorDB.NextMirrorRequest;
	...
	while mMirrorState < oMirrorList.Count do  begin   // more branches to load
		path := oMirrorList[ mMirrorState ];
		if path <> '' then  begin   // skip any blanks
...
procedure  cMirrorDB.AddABranch( const rep : cXmlParser );
	...
	if mMirrorState < msDone then  begin
        pt := FollowPath( TagDataReply + LSep + TagResult + LSep + TagScan, rep.GetRoot ); // rep.FindNodeMulti( rep.GetRoot, TagScan );
        if pt <> nil then  begin
			pResult := pt.Back;
			if {( pResult <> nil ) and ( pResult.NodeName = 'Result' ) and ( pResult.Content = 'OK' ) and}
				( pResult.SubNodes <> nil ) and ( pResult.SubNodes.Count >= 2 ) then  begin
				pPath := FindName( pResult, TagPath );  // .SubNodes[ 1 ];
				if ( pPath <> nil ) and ( pPath.Content = oMirrorList[ mMirrorState ] ) then  begin
					pt := pResult.Back;
					if ( pt <> nil ) and ( pt.SubNodes <> nil ) and ( pt.SubNodes.Count >= 2 ) then  begin
						while pt.SubNodes.Count > 1 do  MoveBranch( pt.SubNodes[ 1 ] );
                        end;
					if mMirrorState < oMirrorList.Count - 1 then  begin
                        Inc( mMirrorState );
                        NextMirrorRequest;
                        end
                    else  begin
                    	mMirrorState := msDone;
                        if Assigned( OnEvent ) then  OnEvent( ceDbReady, '' );
                        Broadcast( '' );
                    	end;
					end;
				end;
...
procedure  cMirrorDB.ProcessJournal;
    ...
    try  begin
        ...
        while not EOF( f ) do  begin
            ReadLn( f, ln );
            if Pos( '<EditReply>', ln ) = 1 then  begin
                xml := ln;
                while not EOF( f ) do  begin
                    ReadLn( f, ln );
                    xml := xml + ln;
                    ...
                    else if ln[ 1 ] <> tab then  break;  // dud request
                    end;
                end;
            end;
    	end;
...
procedure  cMirrorDB.InitJournal;
	...
	var
    	val : string;
	begin
    if HasContent( '|SystemConfig|Journal|Number|', val ) then  begin
        if TryStrToInt( val, mJournalCount ) then  begin
            if not HasContent( '|SystemConfig|Journal|Directory|', mJournalDir ) then  mJournalDir := Directory;
            if not HasContent( '|SystemConfig|Journal|Name|', mJournalName ) then  mJournalName := 'Journal';
            if FileExists( mJournalDir + mJournalName + JournalExt ) then  begin
            	ProcessJournal;
                FlushJournal;
            	end;
...
procedure  cMirrorDB.FlushJournal;
	...
	begin
    if mJournalCount <> 0 then  begin
		try CloseFile( mJournalFile ) except end;   // may not be open
        if CountFiles( mJournalDir, mJournalName + JournalMask ) > mJournalCount then  begin  // too many so replace oldest
            fn := OldestFile( mJournalDir, JournalMask, true );   // true => ignore current journal
            if fn = '' then  fn :=  mJournalName + JournalExt;
            fn := mJournalDir + fn;
        	DeleteFile( PChar( fn ) );
        	end
        else  begin                                                     // one after newest count
            fn := NewestFile( mJournalDir, mJournalName + JournalMask, true );
            if fn = '' then  fn :=  mJournalName + '-' + JournalExt;
        	n := AnyDecimal( fn ) + 1;
            fn := ExtractOnlyFileName( fn );  // core name only eg 'Journal-'
            while FileExists( mJournalDir + fn + IntToStr( n ) + JournalExt ) do Inc( n );
...
function   CollectList( pt : apNode ) : TStringList;
	...
	begin
	if pt <> nil then  Result := StrToPath( pt.Content )
...
function  cMirrorDB.ReadOp( req : cDbTree; pBase : apNode ) : string;
	...
	if pBase <> nil then  begin
        ...
        if pDest <> nil  then  begin
        	d := 1;
        	if pDest.Back <> nil then  begin
                r := StartPath( d, pDest.Back );
                r := r + FormatATag( pDest, d, true );
                end;
            if pDest.SubNodes <> nil then  begin
                for x := 0 to pDest.SubNodes.Count - 1 do begin
                    pt := pDest.SubNodes[ x ];
...
function   cMirrorDB.ScanOp( req : cDbTree; pBase : apNode ) : string;
	...
	if pBase <> nil then  begin
        ...
        if pDest <> nil  then  begin
            d := 1;   result := '';
            if pDest.Back <> nil then  result := StartPath( d, pDest.Back );
            result := result + FormatAllNodes( pDest, d );
            d := 1;
            if pDest.Back <> nil then  result := result + uXmlParser.EndPath( d, pDest.Back );
	        end;
		end;
	end;
...
procedure  cMirrorDB.NewOp( req : cDbTree; pBase : apNode; lPath : TStringList; deleteFirst : boolean = false );
    ...
    if pBase <> nil then  begin
        ...
        if NodeContent( pName ) <> '' then  begin   //  ( pPath <> nil ) and (   )
            ...
            if ( pDest <> nil ) and ( nam <> '' ) then  begin
                ...
                if pNew <> nil then  begin
                    if ( pNew.NodeName <> nam ) and Master then  begin  // auto renamed so fix reply so slaves see same key
    	...
    	else  LogEr( erBadReqNew, 'Invalid ReqNewTag request' );
    	end;
	end;
{<EditRequest>
...
procedure  cMirrorDB.RenameOp( req : cDbTree; pBase : apNode; lPath : TStringList );
    ...
    if pBase <> nil then  begin
        ...
        if NodeContent( pName ) <> '' then  begin  //  ( pPath <> nil ) and (  )
        	pDest := FollowPath( lPath, GetRoot ); // GetNode( NodeContent( pPath ) );
	        nam := MakeTagNameLegal( NodeContent( pName ), true );
            if ( nam <> '' ) and ( pDest <> nil ) and ( pDest.Back <> nil ) then  begin
                if pDest.Back.Table <> nil then  begin   // does hash table need changing
                    if CanAdd( nam, pDest.Back ) then  begin
                        pDest.Back.Table.Delete( pDest );
                        pDest.NodeName := nam;
                        pDest.Back.Table.Add( pDest );   // was   pPath
                        end
                    else  LogEr( erRenToNonuniqueKeyNode, 'RenameOp ' + lPath[ lPath.Count - 1 ] + ' to ' + nam );
                    end
                else  pDest.NodeName := nam;
                Inc( mEditCount );
                pDest.Modified := Seconds;
                end;
	        end
        else  LogEr( erRenToEmpty, 'RenameOp To ""' );
		end
	else  LogEr( erRenNonExistent, 'RenameOp badly formed or Non Existent Path' );
	end;
...
procedure  cMirrorDB.DeleteOp( req : cDbTree; pBase : apNode; lPath : TStringList );
	...
	begin
    if pBase <> nil then  begin
        pDest := FollowPath( lPath, GetRoot );  // GetNode( ReadContent( pBase, TagPath ) );
		if ( pDest <> nil ) and ( pDest.Back <> nil ) then  begin  // don't delete root
            DeleteAllFast( pDest );
            end;
        end
	else  LogEr( erDelNonExistent, 'DeleteOp Non Existent Node' );
	end;


procedure  cMirrorDB.EditOp( req : cDbTree; pBase : apNode; lPath : TStringList );   //  if <Prev> provided it must match old content
    ...
    if pBase <> nil then  begin
        ...
        if pDest <> nil then  begin
            ...
            if pPrev <> nil then  begin  // must have correct old value - external consistency check
...
function   cMirrorDB.HandleADataRequest( req : cDbTree ) : string;
    ...
    if pbase <> nil then  begin   // despatch to appropriate handler
    	if FindName( pbase, TagRead ) <> nil then  result := ReadOp( req, pbase )
        else  if FindName( pbase, TagScan ) <> nil then  result := ScanOp( req, pbase )
...
procedure  cMirrorDB.HandleAEditRequest( req : cDbTree );
    ...
    if pbase <> nil then  begin   // despatch to appropriate handler
        ...
        pPath := FindName( pBase, TagPath );  // check path is mirrored here to exclude extraneous errors
        if pPath <> nil then  begin
            lPath := StrToPath( pPath.Content );
            //if ( lPath <> nil ) and ( lPath.Count > 0 ) then  begin
                if Master or ( oMirrorList.Count > 0 ) and ( ( oMirrorList[ 0 ] = LSep )
                	or ( oMirrorList.Find( lPath[ 0 ], x ) ) ) then  begin  // edit path is mirrored

                    if FindName( pbase, TagEdit ) <> nil then  EditOp( req, pbase, lPath )
                    else if FindName( pbase, TagNewTag ) <> nil then  NewOp( req, pbase, lPath )
                    else if FindName( pbase, TagReplaceTag ) <> nil then  NewOp( req, pbase, lPath, true )
                    else if FindName( pbase, TagDelete ) <> nil then  DeleteOp( req, pbase, lPath )
                    else if FindName( pbase, TagRenameTag ) <> nil then  RenameOp( req, pbase, lPath )
...
procedure	cMirrorDB.GlobalEdit( basePath : string;  NodeName, val : string; id : string = '' );
    ...
    if pn <> nil then  begin   // already exists
    	if ( pn.Content <> val )  then  begin  // ignore nop changes
    		r := FormatShortEditRequest( basePath + NodeName + '|', val, id ); // SysID
            end;
    	end
    else  begin
...
procedure	cMirrorDB.GlobalEdit( pBase : apNode;  NodeName, val : string; id : string = '' );
    ...
    if pn <> nil then  begin   // already exists
    	if ( pn.Content <> val )  then  begin  // ignore nop changes
    		r := FormatShortEditRequest( ResolvePathStr( pBase ) + NodeName, val, id ); // SysID
            end;
    	end
    else  begin
    	r := StartRequestNew( NodeName );
...
procedure  cMirrorDB.GlobalRename( pBase : apNode;  newName : string; id : string = '' );

	var
		r : string;
    begin
    if ( pBase <> nil ) and ( NodeName( pBase ) <> newName ) then  begin   // already exists
    	if id = '' then  id := mID;
    	r := FormatRenameRequest( ResolvePathStr( pBase ), newName, id ); // was SysID
    	if r <> ''  then  BroadcastRequest( r, id );  // mHub.Broadcast( r, nil );
        end;
    end;
...
procedure  cMirrorDB.GlobalDelete( pBase : apNode; id : string = '' );
	...
	var
		r : string;
    begin
    if ( pBase <> nil ) and ( pBase <> GetRoot ) then  begin
    	if id = '' then  id := mID;
...
procedure  cMirrorDB.GlobalFlag( basePath : string; name : string; state : boolean; id : string = '' );
    ...
    if ( basePath <> '' )  then  begin
        ...
        if r <> '' then  BroadcastRequest( r, id );
        end;
    end;


procedure  cMirrorDB.GlobalFlag( pBase : apNode; name : string; state : boolean; id : string = '' );
...
procedure	cMirrorDB.LoadObject( obj : TObject; baseNode : apNode );
    ...
    if baseNode <> nil then  begin
        ...
        for p := 0 to count - 1 do  begin
            ...
            if lPropType^.Kind = tkUString then  SetPropValue( obj, nam, val )
            else if lPropType^.Kind = tkInteger then  begin
                if TryStrToInt( val, v ) then  SetPropValue( obj, nam, v );
                end
            else if lPropType^.Kind = tkEnumeration then  begin    // assume boolean
...
procedure	cMirrorDB.PersistObject( obj : TObject; baseNode : apNode );
    ...
    if baseNode <> nil then  begin
        ...
        for p := 0 to count - 1 do  begin
            ...
            nam := string( lPropInfo.Name );
            lPropType := lPropInfo^.PropType;
            if lPropType^.Kind = tkUString then  GlobalEdit( baseNode, nam, GetPropValue( obj, lPropInfo ) )
            else if lPropType^.Kind = tkInteger then  begin
...
procedure	cMirrorDB.PersistObject( obj : TObject; basePath : string );
    ...
    for p := 0 to count - 1 do  begin
        ...
        nam := string( lPropInfo.Name );
        lPropType := lPropInfo^.PropType;
        if lPropType^.Kind = tkUString then  GlobalEdit( basePath, nam, GetPropValue( obj, lPropInfo ) )
        else if lPropType^.Kind = tkInteger then  begin
		...
		if pb <> nil then  begin
			...
			if pt <> nil then  begin
                ...
                if pt <> nil then  begin
                    ...
                    pUsrName := FollowPath( '|Users|' + nam + LSep, GetRoot );   // NOTE name cannot have a space
                    if pUsrName <> nil then  begin   // user name found in DB

                        pt := req.FindNodeMulti( pb, TagLogout );    // log out
                        if pt <> nil then  begin
                            if oLoggedIn.Find( id, x ) then  begin
                                oLoggedIn.Delete( x );
                                GlobalEdit( pUsrName, 'LoggedOut', ShortDateTime( Now ) + '  from ' + id, SysID );
                            ...
                            pt := req.FindNodeMulti( pb, TagPassWord );         // todo NOTE pw should be pre hashed before here
                            if pt <> nil then  begin
                                pw := pt.Content;
								pt := FindName( pUsrName, TagPassWord );
								if pt <> nil then if pt.Content = pw then  begin  // valid name & password
									pw := '';
									pt := FindName( pUsrName, 'Access' );
									if pt <> nil then  begin
                                    	access := pt.Content;    //  access rights
                                    	if access <> '' then  begin
                                            LogEr( stLogin, 'Login ' + nam + ' ID ' + id );   // if mLog <> nil then  mLog
                                            GlobalEdit( pUsrName, 'LoggedIn', ShortDateTime( Now ) + '  from ' + id, SysID );
                                            GlobalEdit( pUsrName, TagReqID, id, SysID );
							...
							else  if ( pw = SysID ) or ( pw = '_Alphasoft_' ) then  begin
								accept := true;  access := '9,*';  //result := nam;
								end;
							end;
						end;
					if not accept then  begin  // failed attempt
						LogEr( wnLogin, 'FAILED Login attempt ' + nam + ' ID ' + id );
						er := wnLogin;
						end
					else  begin
						er := erNone;
...
procedure  cMirrorDB.SlaveHandler( const reply : string );
	...
	begin
	mEr := 0;
	if Length( reply ) > Length( TagEditReply ) * 4 then  begin
		// intro := uUtils.Slice( reply, 1, Length( TagEditReply ) * 4 );
		if PosN( TagEditReply, reply, 60 ) > 0 then  begin    //  intro
			rep := cXmlParser.Create( 20 );
			rep.LoadFromString( reply );
			// if ResultOK( rep.GetRoot ) then  begin
            edReply := FindName( rep.GetRoot, 'EditReply' );
            if edReply <> nil then  begin
                edResult := FindName( edReply, 'Result' );
                if edResult <> nil then  begin
                     er := AnyDecimal( NodeContent( edResult ) );
                     mLastID := ReadContent( edResult, 'ReqID' );
                     if er = 0 then  begin  // good result pkt so do edit here to achieve mirror
						HandleAEditRequest( rep );
                        if not mLoadingFromJournal then  begin
                            Broadcast( reply );     // distribute change notification locally
                            if mJournalCount <> 0 then  AddToJournal( reply );
                            end;
                		end
                    else if not mLoadingFromJournal and Assigned( ErrorHandler ) then  begin   // respond to incoming error mesg
...
function  cMirrorDB.LocalRequestHandler( const request : string; link : apLinkID ) : string;   // all DBreq comes via here
	...
	var
		...
		ed : boolean;
	begin
	result := '';   req := nil;    ed := false;     mEr := 0;
	if Length( request ) > Length( TagDataRequest ) * 4 then  begin
		...
		if Pos( TagDataRequest, intro ) > 0 then  begin
            ...
            mEr := req.mEr;
			pr := req.GetRoot;
			if ( mEr = 0 ) and ( pr <> nil ) and ( pr.SubNodes <> nil ) and ( pr.SubNodes.Count > 0 ) then  begin
				pNode := pr.SubNodes[ 0 ];  // todo handle multiple requests in one xml ie ACID
				if pNode.NodeName = TagDataRequest then  begin
		...
		if req <> nil then  begin
			result := req.FormatADataReply( ed, mEr ) + response + req.FormatEndDataReply( ed );
			if ed then	begin
            	Broadcast( result );
            	if mJournalCount <> 0 then  AddToJournal( result );
            	end;
            if ( mEr <> 0 ) and ( Assigned( ErrorHandler ) and not mLoadingFromJournal ) then  begin
