...
type
	...
	public
		...
		function Scan(path: TStringList; NextNode: poOnNextNode)
		  : boolean; overload;
		...
		function SetFlag(pn: apNode; const flag: string;
		...
		function FollowPath_(pathl: TStringList; pt: apNode = nil)
		  : apNode; overload;
...
begin
	result := cHashTable<apNode>.Create( function(pt: apNode)
	  : string begin result := pt.NodeName; end, // name match
	  procedure(var pt: apNode)begin pt := nil; end, // clear slot
	  function(pt: apNode): boolean begin result := pt = nil; end);
	// test for clear
end;
...
begin
	result := false;
	Inc(x);
	if (base <> nil) and (base.SubNodes <> nil) and
	  (x < base.SubNodes.Count) then
	begin
		ps1 := base.SubNodes[x];
		result := true;
	end;
				...
				if pt.SubNodes <> nil then
					...
					begin
						...
						begin // slip it into the right spot  todo index or halve interval
							ptb := sl[y];
							if NamedContent(ptb, nam) >
							  NamedContent(pta, nam) then
							begin // insert before
								sl.Insert(y, pta);
								done := true;
								break;
