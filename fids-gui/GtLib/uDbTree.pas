...
type
    ...
    apSortCompareMethod = function( at, bt : apNode ) : integer of object;   // < -1,  = 0,  > +1
	...
	cDbTree = class
		public
			...
			function   ReadNode( pt : apNode; var name : string; var content : string  ) : integer;
			function   ReadSub( pt : apNode; n : integer; var name : string; var content : string ) : apNode;
			function   FindNodeMulti( pt : apNode; const tn : string ) : apNode;  // multi level search from pt/nil to NodeName tn
            function   Search( pt : apNode; name, cont : string; index : int = 1 ) : apNode;  // multi search for matching(s)
			...
			procedure	QuickSort( pt : apNode; L, R : integer; compare: apSortCompareMethod );
			//function   CollectList( pt : apNode ) : TStringList;
			function   NewNode( const nam : string; key : boolean; parent : apNode; AutoInc : int = 0 ) : apNode;  overload;// and add to tree
			function   NewNode( const name, attribs : string; parent : apNode ) : apNode;  overload;
            ...
            function   SetNode( pn : apNode; var nodeName : string;  const val : string ) : apNode;
			procedure  DeleteAllFast( pt : apNode; base : boolean = true );
			function	CountAllSubs( base : apNode ) : integer;
			function   FollowPath_( pathl : TStringList; pt : apNode = nil ) : apNode;  overload;
			...
			function   ListNames( const path : string; pt : apNode = nil ) : TStringList;  // returns a TSlist from sub node names
			...
			function   AddToList( pt : apNode; const Name, SubName, SubCont : string; unique : boolean = false ) : apNode;
	...
	function	FindName( pt : apNode; nam : string ) : apNode;  // single level search ( with hash table )
	...
	function	ParseList( const path : string ) : TStringList;   deprecated;  // see StrToPath  or buildparamL
	...
	function	FollowPath( pathl : TStringList; pt : apNode; back : integer = 0 ) : apNode;  overload;
	...
	function	Traverse( start : apNode; backStep : int; path : TStringList; backAdjust : int = 0 ) : apNode;
    function	EachSubNode( const base : apNode; var x : int; out ps1 : apNode ) : boolean;  // subnode enumerator  start x := -1
	function	UniqueTagName( base : apNode; const SubName : string ) : string;
    function	LoadContentList( const pcl : apNode ) : TStringList;  // creates a list caller must free

    procedure	SetContent( pt : apNode; val : string );  inline;   // LOCAL WRITE FUNCTIONS handle with care
...
var  SortPath : TStringList;   // temp parameter for sort list
...
function   NodeName( pt : apNode ) : string;
	...
	begin
	result := '';
	if pt <> nil then  result := pt.NodeName;
	end;
...
procedure	SetContent( pt : apNode; val : string );
	...
	begin
    if ( pt <> nil ) and ( pt.Back <> nil ) then  pt.Content := val;
    end;
...
procedure	PutOnTop( top : apNode );
	...
	var
        ...
        x : int;
	begin
    if ( top <> nil ) and ( top.Back <> nil ) then    begin
        parent := top.Back;
        for x := 0 to parent.SubNodes.Count - 1 do  begin
            if top = apNode( parent.SubNodes[ x ] ) then  begin
...
function	NodeIndex( pt : apNode ) : int;
	...
	begin
    result := -1;
    if ( pt <> nil ) and ( pt.Back <> nil ) then  begin
...
function   SubNodeCount( pt : apNode ) : int;
	...
	begin
	result := 0;
	if ( pt <> nil ) and ( pt.SubNodes <> nil ) then  result := pt.SubNodes.Count;
	end;
...
function   SubNode( pt : apNode; n : int ) : apNode;
	...
	begin
	result := nil;
	if ( pt <> nil ) and ( pt.SubNodes <> nil ) and ( n < pt.SubNodes.Count ) then  begin
...
function  FindRoot( pt : apNode ) : apNode;
    ...
    while result.Back <> nil do  begin
        result := result.Back;
    	end;
    end;
...
function  FindName( pt : apNode; nam : string ) : apNode;
	...
	begin
	result := nil;
	if ( pt <> nil )  and ( nam <> '' ) then  begin //  and ( pt.SubNodes <> nil )
		if nam[ 1 ] = SubNodeBracket then  begin  // [n]
			x := 2;
			i := GetInt( nam, x );
			if //( i >= 0 ) and ( i < pt.SubNodes.Count ) and
            	( nam[ x ] = SubNodeEndBracket ) then  begin
				result :=  SubNode( pt, i ); // pt.SubNodes[ i ];      // step in
				end
			else result := nil;
			end
		else if nam[ 1 ] = chIndirect then  begin  // ^
            result := FollowPath( NodeContent( pt ), FindRoot( pt ) );
			end
		else if pt.Table <> nil then  begin
...
function CreateHashTable() : cHashTable<apNode>;
	...
	result := cHashTable<apNode>.Create(
		function( pt : apNode ) : string  begin  result := pt.NodeName;  end,  // name match
		procedure( var pt : apNode )  begin  pt := nil;  end,                  // clear slot
		function( pt : apNode ) : boolean  begin  result := pt = nil;  end );  // test for clear
	end;


constructor  cDbTree.Create( treeID : word = 0; isKey : boolean = false; log : aLogProc = nil );
...
procedure  cDbTree.DisposeNodeFast( pt : apNode );
	...
	if pt.ID = 1 then  Dec( mNodeCount );
...
procedure  Unlink( pt : apNode );
	...
	if pt <> nil then  begin
		...
		if ppt <> nil then  begin
			if ppt.SubNodes <> nil then ppt.SubNodes.Delete( ppt.SubNodes.IndexOf( pt ) );   // unlink
...
function	NodeIndexParent( pt : apNode;  out parent : apNode ) : int;
	...
	if pt <> nil then  begin
		...
		if parent <> nil then  begin
			if parent.SubNodes <> nil then result := parent.SubNodes.IndexOf( pt );
			end;
		end;
	end;


procedure	NodeInsert( pt : apNode; base : apNode; index : int );   // assumes non key parent or already in table

	begin
    if ( pt <> nil ) and ( base <> nil ) and ( base.SubNodes <> nil ) and
    	( index >= 0 ) and ( index < base.SubNodes.Count )  then  begin
...
function   IDValue( n : string ) : int;
    ...
    x := Length( n );
    while x > 0 do  begin
    	if not IsDecimal( n[ x ] ) then  break;
...
procedure  cDbTree.IncAutoInc( parent : apNode );
	...
	begin
    if ( parent <> nil ) and ( parent.AutoInc <> 0 ) then  begin
    	Inc( parent.AutoInc );
        if parent.AutoInc > MaxAutoInc then  begin
        	parent.AutoInc := 1;
            if mEr < stAutoIncWrapAround then  begin
...
function  cDbTree.AutoIncName( parent : apNode; const nam : string ) : string;
	...
	begin
    result := nam;
    if parent.AutoInc > 0 then  begin
        if not IsDecimal( nam[ Length( nam ) ] ) then  begin  // if not already numbered
            result := nam + IntToStr( parent.AutoInc );
            n := 0;
            repeat   // step until we find an empty one
                if FindName( parent, result ) = nil then  break;
                IncAutoInc( parent );
                result := nam + IntToStr( parent.AutoInc );
                Inc( n );
            	until n > 1000;
    ...
    if ( parent <> nil ) then  begin
        ...
        if parent.Table <> nil then  begin
            if FindName( parent, nam ) <> nil then  begin   // collides
                result := false;
                if mEr < erNonUniqueNewKeyNode then  begin
                    LogEr( erNonUniqueNewKeyNode, 'Non Unique NewNode( ' + nam + ' ) on a Key tag' );
                    end;
                end
            end;
	    end;
...
procedure  cDbTree.LinkNode( pt, parent : apNode );
    ...
    if parent.SubNodes = nil then  parent.SubNodes := TList.Create; // forward link
    parent.SubNodes.Add( pt );
    if parent.Table <> nil then  parent.Table.Add( pt );
    Inc( mNodeCount );  // if pt.ID = 1 then    // wrong for AddNode
    pt.Modified := Seconds;
    end;


function   cDbTree.NewNode( const nam : string; key : boolean; parent : apNode; AutoInc : int = 0 ) : apNode; // and add to list

	var
		pNode : apNode;
        name : string;
	begin
    if parent = nil then  parent := GetRoot;
    name := AutoIncName( parent, nam );
	result := nil;
    if CanAdd( name, parent ) then  begin
        New( pNode );
        pNode.NodeName := name;
        if name <> nam then  IncAutoInc( parent );

        if key then  pNode.Table := CreateHashTable
        else  		 pNode.Table := nil;
...
function   cDbTree.NewNode( const name, attribs : string; parent : apNode ) : apNode;
	...
	var
        p, auto : int;
	begin
    p := Pos( Attrib_AutoInc, attribs );    auto := 0;
    if p > 0 then  begin
...
function   cDbTree.Rename( pt : apNode; const name : string ) : boolean;
    ...
    result := false;
    pParent := pt.Back;
    if ( pParent <> nil ) and ( CanAdd( name, pParent ) ) then  begin
    	if pParent.Table <> nil then   pParent.Table.Delete( pt );
        pt.NodeName := name;
        if pParent.Table <> nil then   pParent.Table.Add( pt );
        result := true;
    	end;
    end;
...
procedure  cDbTree.MoveSubs( ps, pd : apNode );
	...
	var
		...
		doit : bool;
	begin
	if ( ps <> nil ) and ( ps.SubNodes <> nil ) and ( pd <> nil ) then  begin
        for pt in ps.SubNodes do  begin
            doit := true;
            if Assigned( pd.Table ) then  begin   // already on a key tag ?
                if FindName( pd, pt.NodeName ) <> nil then  doit := false;
                end;
            if doit then  begin
                Unlink( pt );
                AddNode( pt, pd );
...
procedure  cDbTree.CopySubs( const ps : apNode; pd : apNode = nil );
	...
	var
        ...
        x : int;
	begin
    if pd = nil then  pd := GetRoot;

	if ( ps <> nil ) and ( pd <> nil ) then  begin
        x := -1;
        while EachSubNode( ps, x, ps1 ) do  CopyBranch( ps1, pd );
		end;
    end;
...
function  FollowPath( pathl : TStringList; pt : apNode; back : integer = 0 ) : apNode;
	...
	if pathl <> nil then  begin
		result := pt;    n := 0;
		while ( n < pathl.Count - back ) and ( result <> nil )  do  begin
...
function   cDbTree.SetNode( pn : apNode; var nodeName : string;  const val : string ) : apNode;

    var
    	pt : apNode;
	begin
    if pn = nil then  pn := GetRoot;
    pt := FindName( pn, nodeName );
    if pt = nil then  begin  //  need to create one
        pt := NewNode( nodeName, '', pn );
        nodeName := pt.NodeName;  // could be a numbered auto inc
    	end;
    if pt <> nil then  pt.Content := val;
    result := pt;
    end;


function   cDbTree.SetFlag( pn : apNode; const flag : string; state : boolean ) : apNode;
    ...
    if pn <> nil then  begin
        ...
        if state then  begin
        	if result = nil then  result := NewNode( flag, false, pn );
	        end
...
function   cDbTree.AddNode( pNode, parent : apNode ) : boolean;
	...
	begin
	result := false;
    if CanAdd( pNode.NodeName, parent ) then  begin
        LinkNode( pNode, parent );
		Result := true;
		end;
	end;
...
procedure  cDbTree.MoveBranch( pt : apNode );
	...
	if pt <> nil then  begin
		doit := true;
		pr := GetRoot;
		if Assigned( pr.Table ) then  begin
			if FindName( pr, pt.NodeName ) <> nil then  doit := false;
			end;
		if doit then  begin
			Unlink( pt );
			AddNode( pt, pr );
...
procedure  cDbTree.DeleteAllFast( pt : apNode; base : boolean = true );
	...
	if ( pt <> nil ) and ( pt.SubNodes <> nil ) then  begin
		for x := pt.SubNodes.Count - 1 downto 0 do  begin
			pd := pt.SubNodes[ x ];
	...
	if pt <> nil then  begin
		if base then  DisposeNode( pt )       // and unlink
		else          DisposeNodeFast( pt );
		end;
	end;
...
procedure cDbTree.Clear;
	...
	var
		pt : apNode;
	begin
	if mRootNode.SubNodes <> nil then  begin
		for pt in mRootNode.SubNodes do  DeleteAllFast( pt );
...
function   cDbTree.GetNode( const path : string ) : apNode;  // overload;
	...
	begin
	if path <> '' then  result := FollowPath( path, GetRoot )
    else  result := nil;
	end;
...
function   PathToStr( path : TStringList ) : string;
    ...
    if path <> nil then  begin
        for p := 0 to path.Count - 1 do  begin
            if p = 0 then  result :=  LSep + path[ 0 ] + LSep
            else  result := result + path[ p ] + LSep;
        	end;
    	end;
    end;
...
function	StrToPath( path : string ) : TStringList;
	...
	x := 1;     p := path;
    if p = '' then  p := LSep;
    while x <= Length( p ) do  begin  // skip initial whitespace and LSep
        if p[ x ] = LSep then  begin  Inc( x );  break;  end
        else if p[ x ] > ' ' then  break;
        Inc( x );
        end;

    while x <= Length( p ) do  begin   // build list
        if p[ x ] = LSep then  begin
            Result.Add( tg );
            tg := '';
            end
        else tg := tg + p[ x ];
    ...
    if tg <> '' then  Result.Add( tg );   // in case there is no trailing LSep
   	end;
...
function   ResolvePathStr( pt : apNode ) : string;   // breaking change 28 apr 2011 - '|asdf|asdf|' form  was 'adsf|adf|adfs'
...
function	TrimPathToBase( base : asDbNode; var path : TStringList ) : boolean;
	...
	begin
    result := false;
    while ( path.Count > 0 ) and ( base <> path[ 0 ] ) do  begin
        path.Delete( 0 );
    	end;
    if ( path.Count > 0 ) and ( base = path[ 0 ] ) then  begin   // got a match
        path.Delete( 0 );
        result := true;
    	end;
    end;
...
function	PathDif( fromPath, toPath : TStringList; var stepBack : int ) : TStringList;
	...
	var           // assumes paths trimmed to a common base eg 'Timetable' and toPath may be flattened eg '[0]' from DS
    	x, highestMatch : int;
	begin
	x := 0;  stepBack := 0;   highestMatch := -1;
    result := TStringList.Create;
    if ( toPath <> nil ) and ( fromPath <> nil ) then  begin
        while true do  begin
            if x >= fromPath.Count then  break;
            if x >= toPath.Count then  break;
            if fromPath[ x ] = toPath[ x ] then  highestMatch := x;
            if toPath[ x ] = '[0]' then  highestMatch := x;
            Inc( x );
            end;

        stepBack := fromPath.Count - highestMatch - 1;
        x := highestMatch + 1;
        while x < toPath.Count do  begin
            result.Add( toPath[ x ] );
            Inc( x );
            end;
    	end;
...
function	Traverse( start : apNode; backStep : int; path : TStringList; backAdjust : int = 0 ) : apNode;
    ...
    result := nil;
    if ( start <> nil ) and ( path <> nil ) then  begin
        if path.Count <= backAdjust then  result := Back( start, backStep + backAdjust - path.Count )
        else  begin
...
function   EachSubNode( const base : apNode; var x : int; out ps1 : apNode ) : boolean;
    ...
    result := false;
    Inc( x );
    if ( base <> nil ) and ( base.SubNodes <> nil ) and ( x < base.SubNodes.Count ) then  begin
    	ps1 := base.SubNodes[ x ];
        result := true;
	    end;
    end;
...
function   LoadContentList( const pcl : apNode ) : TStringList;  // creates a list caller must free
	...
	begin
    result := nil;
    if ( pcl <> nil ) and ( pcl.Content <> '' ) then  begin
		i := 1;  result := BuildParamsL( pcl.Content, i );
	    end;
    end;
...
function   cDbTree.ReadSub( pt : apNode; n : integer; var name : string; var content : string ) : apNode;

	begin
	if pt = nil then  pt := @mRootNode;
	if ( pt.SubNodes <> nil ) and ( pt.SubNodes.Count >= n ) then  begin
		pt := pt.SubNodes[ n ];
		name := pt.NodeName;
		content := pt.Content;
		end;
...
function   cDbTree.ReadNode( pt : apNode; var name : string; var content : string  ) : integer;
	...
	name := pt.NodeName;
	content := pt.Content;
	if pt.SubNodes <> nil  then  Result := pt.SubNodes.Count;
	end;
...
function   FindSubNode( pt : apNode; const tn : string ) : apNode;  // multi level search from pt to NodeName tn
...
function   cDbTree.FindNodeMulti( pt : apNode; const tn : string ) : apNode;
	...
	begin
	if pt = nil then  pt := @mRootNode;
    if pt.NodeName = tn then  result := pt
    else  begin
        result := FindName( pt, tn );
        if result = nil then  result := FindSubNode( pt, tn );
    	end;
	end;
...
function   cDbTree.ListNames( const path : string; pt : apNode = nil ) : TStringList;
	...
	if pt <> nil then  begin
		result := TStringList.Create;
		if pt.SubNodes <> nil then  begin
			for n := 0 to pt.SubNodes.Count - 1 do  begin
				pn := pt.SubNodes[ n ];
...
function   cDbTree.AddToList( pt : apNode; const Name, SubName, SubCont : string; unique : boolean = false ) : apNode;
    ...
    var
        ...
        rep : boolean;
	begin
    rep := false;    result := nil;
    if pt <> nil then  begin
        pf := FindName( pt, Name );     // create list header if necessary
        if pf = nil then  pf := NewNode( Name, false, pt );  // not key - multiple <file> s
        if unique then  begin
            if ( pf <> nil ) and ( pf.SubNodes <> nil ) then  begin  // search for unique content
                for i := 0 to pf.SubNodes.Count - 1 do  begin
                    pn := pf.SubNodes[ i ];
                    if pn.NodeName = SubName then  begin
                        if pn.Content = SubCont then  begin  // not unique
                            rep := true;
                            break;
                            end;
                        end;
        ...
        if not rep and ( pf <> nil )  then  begin
...
procedure  cDbTree.DelFromList( pt : apNode; const Name, SubName, SubCont : string );
    ...
    if pt <> nil then  begin
        ...
        if ( pf <> nil ) and ( pf.SubNodes <> nil ) then  begin
        	for i := 0 to pf.SubNodes.Count - 1 do  begin  // search for correct item
            	pn := pf.SubNodes[ i ];
                if ( pn.NodeName = SubName ) and ( pn.Content = SubCont ) then  begin
                    DeleteAllFast( pn );
                    break;
                    end;
                end;
...
function  cDbTree.FollowPath_( pathl : TStringList; pt : apNode = nil ) : apNode;
	...
	result := nil;
	if pathl <> nil then  begin
	if pt = nil then  result := @mRootNode  else  result := pt;
		while n < pathl.Count do  begin
			// Result := nil;
			nam := Pathl[ n ];
			if nam <> '' then  result := FindName( result, nam )    // step in
			else               result := nil;
...
function   cDbTree.FollowPath_( path : string; pt : apNode = nil ) : apNode;//  overload;
...
function  cDbTree.ForceEditPath( pathl : TStringList ) : apNode;
	...
	begin
	n := 0;
	if pathl.Count > 0 then  pt := @mRootNode  else  pt := nil;
	while n < pathl.Count do  begin
			nam := pathl[ n ];
...
function   cDbTree.Scan( path : TStringList;  NextNode : poOnNextNode ) : boolean;
	...
	if path <> nil then  pt := FollowPath( path, GetRoot() )
	else                 pt := GetRoot();
	if pt <> nil then  begin
...
function   DoSearch( pt : apNode; name, cont : string; index : int ) : apNode;
    ...
    if (( cont = '' ) or ( cont = pt.Content )) and
       (( name = '' ) or ( name = pt.NodeName )) then  begin
        Dec( index );  if index <= 0 then  result := pt;
    	end;
    if ( result = nil ) and ( pt.SubNodes <> nil ) then  begin
		for i := 0 to pt.SubNodes.Count - 1 do  begin
...
function   cDbTree.Search( pt : apNode; name, cont : string; index : int = 1 ) : apNode;  // multi search for matching(s)
...
procedure  cDbTree.Sort( pt : apNode; pathl : TStringList );
	...
	if ( pathl <> nil ) then if pathl.Count > 0 then  begin
		if pathl.Count = 1 then  begin  // simpler faster case
			sl := TList.Create;
			nam := pathl[ 0 ];
			if pt.SubNodes <> nil then for x := 0 to pt.SubNodes.Count - 1 do  begin
				pta := pt.SubNodes[ x ];   done := false;
		...
		else if pathl.Count > 1 then  begin
            if pt.SubNodes <> nil then  begin
            	SortPath := pathl;
...
procedure	cDbTree.QuickSort( pt : apNode; L, R : integer; compare: apSortCompareMethod );
