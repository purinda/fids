...
uses
	Forms, Generics.Collections, uMessageHub, uMirrorDB, uXmlParser, uGT,
	uGlobalDefs; // uGlobalDefs

type

	aConnectionType = (ctNone, ctStandAlone, ctLocalClient, ctLocalServer,
	  ctRemoteClient, ctServer, ctMultiServer);
	...
	private
		...
		mOriginalDirectory: string;
		// base directory - holds sysconfig and users
		mServerHasShutDown: boolean;
		oReaders: TList<apConnectionEventReader>;
		mRegistryBase: string;
		procedure OnConnectionEvent(event: aConnectionEvent; param: string);
		// called by message hub when connection established
...
begin
	if Xml_Connection = nil then
	begin
		Xml_Connection := cConnection.Create();
		if @reader <> nil then
			Xml_Connection.RegisterEventReader(reader);
	end;
...
begin
	...
	if FileExists(path + 'Config.XML') then
	begin
		if oLocalDB.LoadFromFile(path + 'Config.XML') = nil then
			ShowMessage(AppName + 'Config.XML could not be loaded');
	end
	else if oLocalDB.LoadFromFile('LocalConfig.xml') = nil then
		ShowMessage('LocalConfig not found'); // get local application info
	oLocalDB.HasContent('|LocalConfig|XmlDirectory|', iDB.DB.Directory);
	// get default directory
	if not oLocalDB.HasContent('|LocalConfig|RegistryBase|', mRegistryBase) then
		mRegistryBase := 'Alphasoft\Default';

	machine := oLocalDB.GetNode('|LocalConfig|Machine|');
	// find initialization by machine name or IP addr
	x := -1;
	ID := nil;
	while EachSubNode(machine, x, pt) do
	begin
		if NodeName(pt) = 'IP' then
		begin
			if NodeContent(pt) = ip then
			begin // ip address match so use these id values
				ID := pt;
				break;
			end;
		end;
	...
	if ID = nil then
	begin // try host name match
		x := -1;
		while EachSubNode(machine, x, pt) do
		begin
			if LowerCase(NodeName(pt)) = LowerCase(host) then
			begin
				ID := pt;
				break;
			end;
		...
		if ID = nil then
			ID := oLocalDB.GetNode('|LocalConfig|Machine|Else|');
		// else use defaults
		if ID = nil then
			ID := oLocalDB.GetNode('|LocalConfig|Machine|Default|');
		// else use defaults
	end;
	...
	if ID <> nil then
	begin
		if FindName(ID, 'MultiServer') <> nil then
			ConnectionType := ctMultiServer
		else if FindName(ID, 'SingleServer') <> nil then
			ConnectionType := ctServer
		else if FindName(ID, 'LocalServer') <> nil then
			ConnectionType := ctLocalServer
		else if FindName(ID, 'Client') <> nil then
		begin
			ConnectionType := ctRemoteClient;
			iDB.DB.CanMaster := false;
		end
		else if FindName(ID, 'StandAlone') <> nil then
			ConnectionType := ctStandAlone;
		UserName := ReadContent(ID, 'UserName');
		// not for 'secure' applications
		Password := ReadContent(ID, 'Password');
		iDB.ID := ReadContent(ID, 'LocationID');
		// not for 'secure' applications
	end;

	if oLocalDB.HasContent('|LocalConfig|Server|Port|', value) then
		TryStrToInt(value, PortNo);
	if PortNo = 0 then
		PortNo := DefaultPort;
	if UserName = '' then
		oLocalDB.HasContent('|LocalConfig|UserName|', UserName);
	if Password = '' then
		oLocalDB.HasContent('|LocalConfig|Password', Password);
	if iDB.ID = '' then
		oLocalDB.HasContent('|LocalConfig|LocationID', iDB.ID);
	if iDB.ID = '' then
		iDB.ID := host; // no other name value so settle for PC name
	iDB.DB.ID := iDB.ID; // default id for global edits
	oLocalDB.HasContent('|LocalConfig|Server|Name|', ServerName);

	if ConnectionType <> ctStandAlone then
	begin
...
procedure cConnection.OnConnectionEvent(event: aConnectionEvent; param: string);
// called by message hub when connection established
...
// timeout : boolean;
begin
	if not Assigned(mPrevIdler) then
	begin
		mPrevIdler := Application.OnIdle;
		Application.OnIdle := IdleEventHandler;
		// wait for a quiet moment to announce all clear (all forms should be created)
	end;
end;
...
begin // all forms should be created by now - safe to start talking
	...
	case ConnectionType of
		...
		ctServer:
			begin
				oHub.InitConnection([alLocal, alTCP], ServerName, '',
				  ServerName, tcpMaster, TStringList(nil), PortNo);
			end;
		ctMultiServer:
			begin
				oHub.InitConnection([alLocal, alTCP], ServerName, '',
				  ServerName, tcpMutiMaster, TStringList(nil), PortNo);
			end;
		ctStandAlone:
			begin
				NewConnection;
	...
	begin
		Poller.OnTimeOut(50, // in 5 seconds time check the connection
		  ...
		  if (Xml_Connection <> nil) and not mServerHasShutDown and
		  not oHub.Connected then begin if not oHub.Master then begin
		  ShowMessage('Could not find server ' + ServerName); end; end; end);
	end;
end;
...
begin
	if PosN(MesgShutDown, mesg, 1) = 1 then
	begin // <ShutDown/>
		NotifyEvent(ceShutdown, '');
	end
	else if PosN(MesgEditReply, mesg, 1) = 1 then
	begin
		NotifyEvent(ceEdit, mesg);
	end
	else if PosN(MesgLogin, mesg, 1) = 1 then
	begin // Login
...
begin
	sl := nil;
	x := 1; // load all
	if oLocalDB.HasContent('|LocalConfig|Mirror|', value) then
	begin
		sl := BuildParamsL(value, x);
		found := false;
		for fn in sl do
		begin // make sure SystemConfig is in the list
			if fn = 'SystemConfig' then
			begin
				found := true;
				break;
			end;
...
begin
	...
	begin // no sys config yet
		...
		begin // master only - system initial load from files
			...
			if DB.HasContent('|SystemConfig|XmlDirectory|', value) then
			...
			begin // user set working directory
				...
				name := dir;
				if SelectDirectory(name, [sdAllowCreate, sdPerformCreate,
				  sdPrompt], 0) then
				begin
					name := name + '\';
					if dir <> name then
					begin // new directory selected so save it back to registry
			...
			if DB.HasContent('|SystemConfig|Load|', value) then
			begin // list of xml files to load
				x := 1;
				sl := BuildParamsL(value, x);
				for fn in sl do
				begin
					DB.LoadFromFile(fn); // LOAD FILE LIST
					if DB.HasContent('|LocalConfig|Server|Attended|',
					  value) then
					begin
						if DB.Error <> 0 then
							ShowMessage('Problem loading file ' + fn);
					end;
				end;
				sl.Free;
			end;
...
begin
	...
	begin
		...
		if DB.HasContent('|SystemConfig|Save|', value) then
		begin // list of xml files to save
			x := 1;
			sl := BuildParamsL(value, x);
			for fn in sl do
				DB.SaveDBToFiles(fn);
...
var
	x: int;
begin
	for x := 0 to oReaders.Count - 1 do
	begin
...
begin
	...
	DB.BroadcastRequest(FormatLogIn(UserName, Password, LocationID),
	  LocationID);
end;
...
begin
	...
	if oHub <> nil then
		result := oHub.Master
	else if ConnectionType = ctStandAlone then
		result := true;
end;
