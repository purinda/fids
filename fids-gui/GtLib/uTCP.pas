...
type
	...
	cTCP = class( Tobject )
		...
		private
			...
			mPort : integer;        // Port numbers 1024 through 49151 are the registered ports used for IANA-registered services.
	...
	if con = ctClient then  begin
        ...
        if ip <> '' then  begin  // if mServerIPsL[ x ] <> '' then  begin
            ...
            if mClient.Connected then  begin  // create client reader task
                ...
                mConnected := true;
                mHost := ip;
                if Assigned( mOnConnection ) then  mOnConnection( ceConnected, ip );
                result := true;
                // break;
                end
			else  begin
    ...
    if not mShutdown then  begin
        ...
        if mServer <> nil then  begin
            for p := 0 to mConnections.Count - 1 do  begin
                if mConnections[ p ] <> nil then  begin
                    cp := mConnections[ p ];
                    if ( cp <> nil ) and ( p <> excludeID )  then  begin  //   and ( cp.BytesRead <> 0 )
