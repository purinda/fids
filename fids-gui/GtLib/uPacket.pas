...
function    NewRawPkt( const buf : array of byte ) : apPacket;  // for unformatted incoming
...
procedure	cLockedQueue.WMhandler( var wm : TMessage );  // Main thread only here
	...
	begin
    wm.Result := 0;
    if  wm.Msg = WM_Queue then  begin
    	if Assigned( mReader )  then  mReader( self, wm );   //  and ( wm.Msg = WM_Queue )
    	end
...
procedure  cLockedQueue.Post( pm : apPacket; link : aLink ); // typically reader thread posting incoming to main thread
	...
	Push( pm );
	oLock.Release;
    if pm.pkSize > 1000 then  begin
         mWinMesgNo := mWinMesgNo;
    	end;
    if mhWindow <> 0 then  begin
		if not PostMessage( mhWindow, mWinMesgNo, Ord( link ), 0 ) then // signal main thread to empty the buffer
        	ShowMessage( 'PostMessage fail ' + IntToStr( GetLastError ) + ' : ' + IntToStr( mhWindow ) );
	    end;
	end;
