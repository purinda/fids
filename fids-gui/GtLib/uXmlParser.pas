...
{	eg
    ...
    Hub.Broadcast( EndRequestNew( pArrDep.NodeName, FeedID ) );    // or DB.BroadcastRequest
...
function  cXmlParser.LoadFromFile( fil : string ) : apNode;
	...
	if mEr > 0 then  begin
		...
		if mEr <> 0 then  begin
        	LogEr( erFailedToLoadBackupFile, 'Failed To Load Backup File ' + BackUpFileName( fil ) );
            mLoadBase := nil;
        	end;
		end;
    result := mLoadBase;
...
procedure  cXmlParser.SaveDBToFile( pt : apNode; const FileName : string = '' );
    ...
    if pt <> nil then  begin
        if FileName <> '' then fn := FileName  else  fn := pt.NodeName;
        if Directory <> '' then  begin
        	if Directory[ Length( Directory ) ] <> '\' then  fn := Directory + '\' + fn   // RTL style
            else   fn := Directory + fn;
            end;
        if fn <> '' then  begin
