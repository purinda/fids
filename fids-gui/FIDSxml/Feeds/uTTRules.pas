...
type
    ...
    aTTValidationError = ( teNone, teNoRuleName, teNonuniqueRuleName, teInvalidTime, teInvalidFlight,
    	teAttemptToChangeRuleName,
        teTimeMustBeNumeric, teTimeMinutesMustBeLessThan60, teTimeHoursMustBeLessThan24 );
...
type
	cTTRule = class( TInterfacedObject, IComparer<apNode> )
		...
		public
            ...
            function   Compare( const pfltA, pfltB : apNode ) : int;  // IComparer<apNode>  ie can compare rules for sort
            procedure  Delete;
            function   New : boolean;

        	property   Log : aLogProc  read mLog  write mLog;
        	property   Presentation [ field : aTimeTableField ] : string  read GetField  write SetField;  default; // db Update
        	property   DBTag [ field : aTimeTableField ] : string  read GetDBTagName; //    use var	DBTag [ field ]  below
        	property   Title [ field : aTimeTableField ] : string  read GetTitleField;
        	property   Raw [ field : aTimeTableField ] : string  read GetRawField;

        	property   DbNode : apNode  read mDbNode  write  SetDbNode;   // eg |Timetable|<GT4444>| ie rule name base tag
...
//        end;
	...
	cTTRulesList = class( TList< apNode > )  // < aTTRuleRecord >
        ...
        public
            oRules : cTTRule;
            procedure Build(  field : aTimeTableField; const val : string );  // build a list now where field = val
			procedure Sort( const field : aTimeTableField; const ff : aFlightField; ascend : boolean );
...
function 	cTTRule.MakeValid( field : aTimeTableField ) : boolean;
    ...
    if field in TimeFields then  begin
    	result := false;
        if Length( mVals[ field ] ) = 3 then  Insert( '0', mVals[ field ], 1 );
        x := 1;  v := Decimal( mVals[ field ], x );
        if x <> 5 then  Error := teTimeMustBeNumeric
        else if v mod 100 >= 60 then  Error := teTimeMinutesMustBeLessThan60
        else if v > 2359 then  Error := teTimeHoursMustBeLessThan24
    	else  result := true;
    	end;
    end;
...
procedure   cTTRule.SetField( field : aTimeTableField; val : string );
    ...
    if MakeValid( field ) then  begin
        if mDbNode <> nil then  begin  // not building a new rule so do live updates
            if mVals[ field ] <> Presentation[ field ] then  begin  // changed
                if field in PrimaryFields then  begin
                    mDB.GlobalEdit( mDbNode, TTTagName[ field ], mVals[ field ], mReqID );
                    end
                else if field in RulesSubField then  begin
                    pRules := FindName( mDbNode, TTTagName[ tfRules ] );
                    mDB.GlobalEdit( pRules, TTTagName[ field ], mVals[ field ], mReqID );
                    end
                else if field = tfRuleName then  Error := teAttemptToChangeRuleName;
                end;
            end;
    	end;
    end;
...
procedure   cTTRule.SetDbNode( node : apNode );
    ...
    mDbNode := node;
    pt := FindName( node, TTTagName[ tfTemplate ] );   // set template to rule's primary flight
...
function   cTTRule.Compare( const pfltA, pfltB : apNode ) : int;  // IComparer<apNode>  ie can compare rules for sort
...
function   cTTRule.New : boolean;
    ...
    label
    	DontBother;
    begin
    result := false;    ruleBranch := nil;
    name := mVals[ tfRuleName ];
    if name = '' then  begin  Error := teNoRuleName;  goto DontBother;  end;
    if not MakeValid( tfTime ) then  begin  Error := teInvalidTime;  goto DontBother;  end;
    if FollowPath( tagTimeTable + '|' + name, mDB.GetRoot ) <> nil then  begin
    ...
    for field := Succ( Low( aTimeTableField ) ) to High( aTimeTableField ) do  begin
        if field in PrimaryFields then  begin
        	if not MakeValid( field ) then  goto DontBother;  end;
    		prim := ruleBranch.NewNode( TTTagName[ field ], false, pRulekey );   // add <Rules>, <Path>, <Template>
            prim.Content := mVals[ field ];
            case field of
            tfRules :  begin
                for f := Succ( Low( aTimeTableField ) ) to High( aTimeTableField ) do  begin
                    if f in RulesSubField then  begin  // <DateStart> etc
                        if not MakeValid( f ) then   goto DontBother;
                        sec := ruleBranch.NewNode( TTTagName[ f ], false, prim );
                        sec.Content := mVals[ f ];
                        end
                    end;
                end;
            tfTemplate :  begin
                if not oTemplate.AddFlightToTree( ruleBranch, prim, StripTail( name ) ) then  begin
...
procedure   cTTRulesList.Build(  field : aTimeTableField; const val : string );
    ...
    x := -1;
    while EachSubNode( ptt, x, ptr ) do  begin  // each rule
...
procedure cTTRulesList.Sort( const field : aTimeTableField; const ff : aFlightField; ascend : boolean );
...
function  cTTRulesList.GetEnumerator : TEnumerator< apNode >;  // supports compiler  'for pFlt in FlightList do'
...
var field : aTimeTableField;

initialization

	// generate tag names from enum
    for field := Succ( Low( aTimeTableField ) ) to High( aTimeTableField ) do  begin
        cTTRule.TTTagName[ field ] := EnumToStr( Ord( field ), System.TypeInfo( aTimeTableField ), false );   // was FieldName
        cTTRule.TTTagName[ field ] := UnCamel( cTTRule.TTTagName[ field ], #0 );  // no spaces
        end;
