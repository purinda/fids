...
type
	...
	cDbTree = class
		public
			...
			function   FollowPath_( pathl : TStringList; pt : apNode = nil ) : apNode;  overload;   deprecated; // use fn
			function   FollowPath_( path : string; pt : apNode = nil ) : apNode;  overload;  deprecated; // use fn
    ...
    function	Back( pt : apNode; n : int ) : apNode;
    ...
    function	EachSubNode( const ps : apNode; var x : int; out ps1 : apNode ) : boolean;  // subnode enumerator  start x := -1
...
function   EachSubNode( const ps : apNode; var x : int; out ps1 : apNode ) : boolean;

	begin
    result := false;
    Inc( x );
    if ( ps <> nil ) and ( ps.SubNodes <> nil ) and ( x < ps.SubNodes.Count ) then  begin
    	ps1 := ps.SubNodes[ x ];
        result := true;
	    end;
    end;
