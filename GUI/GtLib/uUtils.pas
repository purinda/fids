...
type
  ...
  public
    ...
    function Read(var ch: char): boolean; virtual; abstract;
    // override;   // next char
...
function GetHex(const s: string; var index: Integer): int;
// F00BAA or , $F00BAA
...
function StrX(const s: string; const x: int): char; inline;
...
// UTF8 convertion on byte stream
...
// YYYYMMDD format eg 20130930 so string sort = date sort
...
function BuildParamsL(const params: string; var index: int; sep: char = ListSep)
  : TStringList;
...
function EnumToStr(val: Integer; pEnumTypePtr: PTypeInfo;
  unCaml: boolean = true): string; // eg TypeInfo( aSortPref )
...
function TimeDateToStr(t: TDateTime; fmt: string = ''): string;
// 'hh:nn ddd d mmm yy'
...
procedure SetEmptyPCharLength(var s: string; len: card);
// see ComputerName below
...
function NewestFile(directory, match: string;
  numbered: boolean = false): string;
function SlashDirectory(directory: string): string;
function OldestFile(directory, match: string;
  numbered: boolean = false): string;
function CountFiles(directory, match: string; numbered: boolean = false): int;
procedure ScanDirectory(directory, match: string; OnEachFile: aOnEachFile;
  subs: boolean = true);
...
uses
  Windows, ShellAPI, Character, Winsock, ShlObj, Registry, ASCII, Dialogs;
// Forms,
...
var
  ...
  skip, c: Integer;
begin
  if mDirection <> sdRead then
  begin // self initialize     InStream := TStringStream.Create(S, TEncoding.UTF8);
    mDirection := sdRead;
    ...
    try
      try
        ...
        mEncoding := enUTF8;
        if (c >= 2) and (buf[0] = $FF) and (buf[1] = $FE) then
        begin // BOM = U+FEFF
          skip := 2;
          mEncoding := enUTF16;
        end
        else if (c >= 2) and (buf[0] >= Ord(' ')) and (buf[1] = 0) and
          (buf[3] = 0) then
        begin // '<?xml.....    ( buf[ 0 ] = Ord( '<' ) ) and
          mEncoding := enUTF16;
        end
        else if (c >= 3) and (buf[0] = $EF) and (buf[1] = $BB) and (buf[2] = $BF)
        then
        begin // BOM = U+FEFF
          skip := 3;
          mEncoding := enUTF8;
        ...
        ShowMessage('could not open ' + mName); // $$$
...
begin
  ...
  if (ks[vk_Shift] and $80) <> 0 then
    Include(result, ksShift);
  if (ks[vk_Menu] and $80) <> 0 then
    Include(result, ksAlt);
  if (ks[vk_Control] and $80) <> 0 then
    Include(result, ksControl);
end;
...
begin
  while (Length(s) > 0) and not IsAlpha(s[1]) do
    Delete(s, 1, 1);
end;
...
begin
  while (Length(s) > 0) and not IsAlpha(s[Length(s)]) do
    Delete(s, Length(s), 1);
end;
...
begin
  result := '';
  x := index;
  while (x <= Length(s)) and ((s[x] = ListSep) or (s[x] = ' ')) do
  begin
    Inc(x);
  end;
  if StrX(s, x) = StringDelim then
  begin
    repeat
      result := result + s[x];
      Inc(x);
      c := StrX(s, x);
      if c = StringDelim then
      begin
        result := result + c;
        Inc(x);
        break
...
begin
  ...
  begin
    while (x <= Length(s)) and (s[x] <> ListSep) do
    begin
      result := result + s[x];
      Inc(x);
    end;
...
begin
  result := 0;
  if (substr <> '') and (Length(src) >= Length(substr)) then
  begin
    f := substr[1];
    result := 0;
    if Length(src) < Length(substr) + limit - 1 then
      limit := Length(src) - Length(substr) + 1;

    for i := 1 to limit do
    begin
      if src[i] = f then
      begin // first char match
        found := true;
        for x := 2 to Length(substr) do
        begin
          if src[i + x - 1] <> substr[x] then
          begin
            found := false;
            break;
          end;
...
begin
  ...
  if x = -1 then
    x := Length(s);
  if (Length(s) > 0) and (Length(s) >= x) then
    result := s[x]; // Length( s ) ];
end;
...
begin
  result := '';
  x := index;
  if (x <= Length(s)) and (s[x] = ListSep) then
  begin
    Inc(x);
  end;
  while (x <= Length(s)) and (s[x] <> ListSep) do
...
begin
  result := 0;
  if GetHexNibble(StrX(s, x), val) then
  begin
    result := val;
    Inc(x);
    if GetHexNibble(StrX(s, x), val) then
...
begin
  result := 0;
  x := index;
  while (x <= Length(s)) and ((s[x] <= ' ') or (s[x] = ListSep) or
    (s[x] = '$')) do
  begin
    Inc(x);
  end;
...
begin
  ...
  x := index;
  negate := false;
  while (x <= Length(s)) and ((s[x] <= ' ') or (s[x] = ListSep)) do
  begin
    Inc(x);
  end;
  if x <= Length(s) then
  begin
    ...
    begin
      while (x <= Length(s)) and (s[x] >= '0') and (s[x] <= '9') do
      begin
...
begin
  ...
  if x <= Length(s) then
  begin
    while (x <= Length(s)) and (s[x] >= '0') and (s[x] <= '1') do
    begin
...
begin
  ...
  while (index <= cardinal(Length(s))) and
    ((s[index] = ' ') or (s[index] = ',')) do
  begin
    Inc(index);
  end;
  while (index <= cardinal(Length(s))) and (s[index] >= '0') and
    (s[index] <= '9') do
  begin
...
begin
  result := 0;
  while (index <= Length(s)) and ((s[index] = ' ') or (s[index] = ',')) do
  begin
    Inc(index);
  end;
  while (index <= Length(s)) and (s[index] >= '0') and (s[index] <= '9') do
  begin
    result := result * 10 + Ord(s[index]) - Ord('0');
    Inc(index);
  end;
...
begin
  if not show0 and (amt = 0) then
    result := ''
  else
    result := Format('%n', [amt]); // was %f eg -1,234.56
end;
...
begin
  result := true;
  if fs.Read(b, 1) = 1 then
  begin
    if b and $80 = 0 then
    begin // top bit zero so just 7 bit ascii
      ch := char(b);
    end
    else if b and $E0 = $C0 then
    begin // 2 byte form   11 bits
      val := (b and $1F) shl 6;
      if fs.Read(b, 1) = 1 then
      begin
    ...
    begin // 3 byte form   goes up to 16 bits
      val := (b and $0F) shl 12;
      if fs.Read(b, 1) = 1 then
      begin
    ...
    // end
      ...
      result := false; // illegal 4/5 byte form   ie EOF
  end
  else
    result := false;
end;
...
begin
  ...
  if p > 0 then
  begin // worth testing
    if (p = 1) or (list[p - 1] = ListSep) then
    begin
      p := p + Length(item);
      if (p > Length(list)) or (list[p] = ListSep) then
        result := true;
    end;
  end;
end;
...
begin
  ...
  it := LowerCase(item);
  p := 1;
  sl := BuildParamsL(list, p);
  for p := 0 to sl.count - 1 do
  begin
    if it = LowerCase(sl[p]) then
    begin
      result := p;
      break;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      trailingCh := true; // catch any unterminated line
    Inc(x);
  end;
  if trailingCh then
    Inc(result);
...
var
  M, D: int;
begin
  for M := src.count - 1 downto 0 do
  begin
    for D := src.count - 1 downto M + 1 do
    begin
      if src[M] = src[D] then
        src.Delete(D);
    end;
...
begin
  ...
  result := '';
  first := true;
  while (x <= Length(n)) and (IsLowerCase(n[x])) do
    Inc(x); // throw leading lowers
  while x <= Length(n) do
  begin
    if IsUpperCase(n[x]) and not IsUpperCase(StrX(n, x - 1)) then
    begin
      if not first and (gapChar <> #0) then
        result := result + gapChar;
      first := false;
    end;
    result := result + n[x];
    Inc(x);
...
function EnumToStr(val: Integer; pEnumTypePtr: PTypeInfo;
  unCaml: boolean = true): string; // stolen from Csi

var // eg str := EnumToStr( Ord( GetSortPref ), TypeInfo( aSortPref ) );
  lEnumDataPtr: PTypeData;
begin
  result := '';
  if pEnumTypePtr.Kind = tkEnumeration then
  begin
    lEnumDataPtr := GetTypeData(pEnumTypePtr);
    if (val >= lEnumDataPtr.MinValue) and (val <= lEnumDataPtr.MaxValue) then
    begin
      result := GetEnumName(pEnumTypePtr, val);
      if unCaml then
        result := UnCamel(result);
    end;
  end;
end;
...
begin
  ...
  lEnumDataPtr := GetTypeData(pEnumTypePtr);
  for x := lEnumDataPtr.MinValue to lEnumDataPtr.MaxValue do
  begin
...
begin
  ...
  begin
    ...
    for x := lEnumDataPtr.MinValue to lEnumDataPtr.MaxValue do
    begin
      if nam = UnCamel(GetEnumName(pEnumTypePtr, x)) then
      begin
        result := x;
        break;
      end;
...
var
  ...
  begin
    while (ips[x] = ' ') and (x <= Length(ips)) do
      Inc(x);
  end;
  ...
  var
    ...
    i: int;
  begin
    while (ips[x] >= '0') and (ips[x] <= '9') and (x <= Length(ips)) do
    begin
      b := b + ips[x];
      Inc(x);
    end;
...
begin // 192.168.0.20-23, 192.168.0.34 -> list of ints
  ipl := TList.Create;
  x := 1;
  while x <= Length(ips) do
  begin
    ip := GetIP();
    ipl.Add(pointer(ip));
    SkipSpace;
    if x <= Length(ips) then
    begin
      if ips[x] = '-' then
      begin
        Inc(x);
        ...
        if ip2 <= ip then
          raise Exception.Create('Invalid IP range');
        repeat
          Inc(ip);
          ipl.Add(pointer(ip));
        until ip = ip2;
...
procedure SetEmptyPCharLength(var s: string; len: card);
// use a string as an empty pchar buffer, see ComputerName below
...
procedure PCharToStr(var s: string; len: card);
// convert PChar buffer back to string, see ComputerName below

var
  i: card;
begin
  for i := 1 to cardinal(Length(s)) do
  begin
    if s[i] = #0 then
    begin // strlen()
      SetLength(s, i - 1);
...
begin
  ...
  IPaddr := '';
  SetLength(AnsiHostName, 256);
  AnsiHostName[1] := #0;
  if GetHostName(@AnsiHostName[1], 256) = 0 then
  // GetHostName(HName^, SizeOf(Name)) = 0 then
  begin
    for i := 1 to cardinal(Length(AnsiHostName)) do
    begin
      if AnsiHostName[i] = #0 then
      begin // strlen()
...
begin
  ...
  FileAttrs := faAnyFile;
  if SysUtils.FindFirst(p, FileAttrs, sr) = 0 then
  begin
    repeat
...
begin
  result := CreateFile(PChar(name), GENERIC_READ, FILE_SHARE_READ, Nil,
    OPEN_EXISTING, 0, 0);
...
begin
  result := CreateFile(PChar(name), GENERIC_WRITE, FILE_SHARE_READ, Nil,
    CREATE_ALWAYS, 0, 0);
end; // use CloseHandle
...
var
  ...
  newest: TDateTime;
begin
  if FindFirst(SlashDirectory(directory) + match, $80, fil) = 0 then
  begin // faNormal
    newest := 0;
    repeat
      if fil.TimeStamp > newest then
      begin
        if not numbered or FileNameHasDecimal(fil.name) then
        begin
          newest := fil.TimeStamp;
          result := fil.name;
        end;
      end;
    until (FindNext(fil) <> 0)FindClose(fil);
  end;
end;
...
var
  ...
  newest: int;
begin
  if SysUtils.FindFirst(SlashDirectory(directory) + match, $80, fil) = 0 then
  begin // faNormal
    newest := 0;
    repeat
      if fil.Time > newest then
      begin
        if not numbered or FileNameHasDecimal(fil.name) then
        begin
          newest := fil.Time;
...
begin
  result := 0;
  if SysUtils.FindFirst(SlashDirectory(directory) + match, faNormal, fil) = 0
  then
  begin // faNormal = $80
    repeat
      if not numbered or FileNameHasDecimal(fil.name) then
...
begin
  ...
  if SysUtils.FindFirst(dir + match, faNormal, fil) = 0 then
  begin // $80 faNormal
    repeat
  ...
  if subs then
  begin
    if SysUtils.FindFirst(dir + '*.*', faDirectory, fil) = 0 then
    begin
      repeat
        if (fil.name <> '.') and (fil.name <> '..') then
        begin
          ScanDirectory(dir + fil.name, match, OnEachFile, subs);
          // if fil.Attr = faNormal then
        end;
      until (SysUtils.FindNext(fil) <> 0);
    end;
  end;
...
begin
  i := 0;
  result := '';
  while (i < High(buf)) and (buf[i] <> nul) do
  begin
    result := result + buf[i];
    Inc(i);
  end;
...
begin
  result := '';
  for x := 0 to path.count - 1 do
  begin
    result := result + path[x];
    if x <> path.count - 1 then
      result := result + '\';
  end;
end;
