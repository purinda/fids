	...
	begin // self initialize     InStream := TStringStream.Create(S, TEncoding.UTF8);
		...
		try
			try
				...
				else if (c >= 3) and (buf[0] = $EF) and (buf[1] = $BB) and
				  (buf[2] = $BF) then
				begin // BOM = U+FEFF
					skip := 3;
					mEncoding := enUTF8;
...
begin
	...
	begin
		...
		if (val >= lEnumDataPtr.MinValue) and
		  (val <= lEnumDataPtr.MaxValue) then
		begin
...
begin
	result := 0;
	if SysUtils.FindFirst(SlashDirectory(directory) + match, faNormal,
	  fil) = 0 then
	begin // faNormal = $80
		repeat
			if not numbered or FileNameHasDecimal(fil.Name) then
