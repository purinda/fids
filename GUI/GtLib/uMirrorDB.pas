  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (pPath <> nil) and (pPath.Content = oMirrorList[mMirrorState]) then
        begin
          pt := pResult.Back;
          if (pt <> nil) and (pt.SubNodes <> nil) and (pt.SubNodes.Count >= 2)
          then
          begin
...
begin
  ...
  begin
    ...
    begin
      if not HasContent('|SystemConfig|Journal|Directory|', mJournalDir) then
...
begin
  ...
  begin
    ...
    if CountFiles(mJournalDir, mJournalName + JournalMask) > mJournalCount then
      ...
      begin
        ...
        begin // does hash table need changing
          ...
          begin
            ...
            LogEr(erRenToNonuniqueKeyNode, 'RenameOp ' + lPath[lPath.Count - 1]
              + ' to ' + nam);
        end
        else
          pDest.NodeName := nam;
...
begin
  ...
  begin // already exists
    if (pn.Content <> val) then
    begin // ignore nop changes
      r := FormatShortEditRequest(ResolvePathStr(pBase) + NodeName, val, id);
      // SysID
    end;
  end
  else
        ...
        begin
          ...
          begin // user name found in DB
            ...
            begin
              ...
              begin
                oLoggedIn.Delete(x);
                GlobalEdit(pUsrName, 'LoggedOut', ShortDateTime(Now) + '  from '
                  + id, SysID);
                ...
                if pt <> nil then
                  ...
                  begin // valid name & password
                    ...
                    begin
                      ...
                      if access <> '' then
                      begin
                        LogEr(stLogin, 'Login ' + nam + ' ID ' + id);
                        // if mLog <> nil then  mLog
    ...
    begin // intro
      ...
      begin
        ...
        begin
          ...
          else if not mLoadingFromJournal and Assigned(ErrorHandler) then
...
begin
  ...
  begin
    ...
    begin
      ...
      if (mEr <> 0) and (Assigned(ErrorHandler) and not mLoadingFromJournal)
      then
      begin
