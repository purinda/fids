...
type
  ...
  public
    ...
    { Basic functionality }
    procedure PopulateGrid(var Force: Boolean);
    procedure PopulateTTGrid();
    procedure fOnIdle(Sender: TObject; var Done: Boolean);
    procedure SetDetail(flightPath: string; ff: aFlightField; detail: string);
...
begin
  ...
  begin
    ...
    begin
      SensorCount := (self.GetSensorCount(self.StringToSensorType(SensorType)));
      ...
      begin
        ...
        begin
          ...
          // if () then
          if (uCommon.isSensorDown(self.GetSensorData
            ((self.StringToSensorType(SensorType)), Status, i)) = 0) then
          begin
            Sensor.Color := clRed;
          ...
          Sensor.Hint := 'Name: ' + self.GetSensorData
            ((self.StringToSensorType(SensorType)), Name, i) + #10#13 + 'IP: ' +
            self.GetSensorData((self.StringToSensorType(SensorType)), IP, i) +
            #10#13 + 'Status: ' + SensorStatus + #10#13 + Location;
...
procedure CFlightController.PopulateGrid(var Force: Boolean);
var
  p: apNode;
...
begin
  ...
  // Exit if table is popuplated
  if (Length(Table) > 1) AND (Force = False) then
  begin
    Exit;
  end;
  ...
  if oFlight = nil then
  begin
    oFlight := cFlight.Create(DB, Xml_Connection.iDB.ID);
  end;
  ...
  begin
    ...
    Table[r].ScheduledCheckinCounters := oFlight.Presentation
